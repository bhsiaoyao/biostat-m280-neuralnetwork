...
```{r}
    input_shape <- dim(trainObs)[-1]
    num_classes <- dim(trainY)[2]
    filters_2 <- 25     # number of convolutional filters to learn
    kernel_size_2 <- 10  # how many time-steps each conv layer sees.
    dense_size_2 <- 40  # size of our penultimate dense layer. 
...
```{r}
    ...
    # Load our best model checkpoint
    bestModel <- load_model_hdf5("best_model_2.h5")
    tidyPredictionProbs <- bestModel %>% 
      predict(testObs) %>% 
      as_data_frame() %>% 
      mutate(obs = 1:n()) %>% 
